...
export default class SquareTemplate {
	...
	static init() {
		SETTINGS.register(SquareTemplate.FIX_ROTATION_PREF, {
			...
			default: true,
			onChange: (toggled) =>
				toggled ? SquareTemplate.patch() : SquareTemplate.unpatch(),
		});
	...
	private static patch() {
		...
		libWrapper.register(
			...
			SquareTemplate.MeasuredTemplate_refreshRulerText,
			"WRAPPER"
		);
	}
	...
	static MeasuredTemplate_getRectShape(
		...
		const matrix = PIXI.Matrix.IDENTITY.rotate(
			-45 * (Math.PI / 180) + direction
		);
	...
	private static MeasuredTemplate_refreshRulerText(
		this: MeasuredTemplate,
		wrapped: () => void
	): void {
		wrapped();
		...
		if (this.document.t === "rect") {
			...
			const size = Math.sqrt(
				(this.document.distance * this.document.distance) / 2
			).toFixed(1);
